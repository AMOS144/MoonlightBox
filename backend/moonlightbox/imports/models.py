...
class ImportSource(Base):
    ...
    __table_args__ = (
        UniqueConstraint("project_id", "preview_id"),
        UniqueConstraint(
            "id",
            "project_id",
            name="uq_import_sources_id_project_id",
        ),
    )
...
class ParticipantAlias(Base):
    ...
    origin: Mapped[str] = mapped_column(String(40), default="quoted_sender")
    resolution_status: Mapped[str] = mapped_column(String(20))
    source_message_ids: Mapped[list[str]] = mapped_column(JSON)
    quoted_message_ids: Mapped[list[str]] = mapped_column(JSON)
...
class Message(Base):
    ...
    id: Mapped[str] = mapped_column(String(36), primary_key=True, default=lambda: str(uuid4()))
    project_id: Mapped[str] = mapped_column(ForeignKey("projects.id", ondelete="CASCADE"))
    import_id: Mapped[str] = mapped_column(ForeignKey("import_sources.id", ondelete="CASCADE"))
    participant_id: Mapped[str] = mapped_column(ForeignKey("participants.id", ondelete="RESTRICT"))
