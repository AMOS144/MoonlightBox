...
export function userMessage(value: unknown, status?: number, code?: string): string {
  ...
  if (code === 'quota_exhausted') return '模型套餐用量已用完：升级套餐或购买积分，或等待下一计费窗口后再重试。'
  if (code === 'billing_unavailable') return '模型账户余额不足，请充值后再重试。'
  if (code === 'rate_limited') return '模型服务请求频率超限，稍等片刻再重试即可。'
  if (code === 'provider_input_rejected') return '模型服务判定输入内容涉敏，进度已保留，调整内容后可重试。'
  if (code === 'provider_output_rejected') return '模型服务判定生成内容涉敏，进度已保留，可以重试或调整要求。'
  if (code === 'authentication') return '模型服务未能验证凭据，请检查设置中的 API Key。'
  if (code === 'preparation_failed') return '准备未完成，已有进展保留，可以重试。'
