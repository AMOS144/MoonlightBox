...
function StickerMessage({ mediaUrl }: { mediaUrl: string }) {
  const [failed, setFailed] = useState(false)
  // 保留明确的资源错误，不把缺图静默当作没有发送。
  return failed ? (
    <span role="status">表情包加载失败</span>
  ) : (
    <img alt="表情" className="chat-sticker" src={mediaUrl} onError={() => setFailed(true)} />
  )
}
...
function MessageContent({
  ...
  if (type === 'sticker') {
    return mediaUrl ? <StickerMessage key={mediaUrl} mediaUrl={mediaUrl} /> : <span role="status">表情包资源缺失</span>
  }
